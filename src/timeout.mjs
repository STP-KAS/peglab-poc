...
export function inspectLock(lock) {
  return {
    ...
    warning: 'TIMEOUT ESCROW. CLAIM NOW OR RECLAIM AFTER tx.time. NOT USD.',
  };
}
...
export function timeoutDemo({path = 'claim'} = {}) {
  ...
  steps.push({title: 'Lock 0.2 tKAS, 48h', status: lock.status, lesson: 'Recipient may claim immediately. Sender waits.'});
